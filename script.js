...
// first name request
while (true) {
  profile.first_name = prompt("What is your name? 😇");
...
// last name request
while (true) {
  profile.last_name = prompt("What is your last name? 🙃");
...
//gender request 
while (true) {
  const gender = prompt("What is your gender? 🌸 (M/F/X)").toUpperCase();
  if (["M", "F", "X"].includes(gender)) {
    profile.gender = gender;
    break;
  }
  alert("Please enter a valid gender (M, F, or X).");
}

// gender interests request
while (true) {
  const genderInterest = prompt("What gender are you interested in? 🍒 (M/F/X)").toUpperCase();
  if (["M", "F", "X"].includes(genderInterest)) {
    profile.gender_interest = genderInterest;
    break;
  }
  alert("Please enter a valid gender interest (M, F, or X).");
}

// location request
while (true) {
  const location = prompt("Where do you live? 🌎 (rural/city)").toLowerCase();
  if (["rural", "city"].includes(location)) {
    profile.location = location;
    break;
  }
  alert("Please enter a valid location (rural or city).")
}

// min and max age request 
let minAge, maxAge;

while (true) {
  minAge = prompt("Please enter the minimum age you're interested in:");
  if (!isNaN(minAge) && Number(minAge) >= 18) {
    profile.min_age_interest = Number(minAge); 
    break;
  }
  alert("Please enter a valid minimum age (18 or older).");
}

while (true) {
  maxAge = prompt("Please enter the maximum age you're interested in:");
  if (!isNaN(maxAge) && Number(maxAge) >= 18) {
    if (Number(maxAge) >= profile.min_age_interest) {
      profile.max_age_interest = Number(maxAge); 
      break;
    } else {
      alert(`The maximum age must be greater than or equal to ${profile.min_age_interest}.`);
    }
  } else {
    alert("Please enter a valid maximum age (18 or older).");
  }
}
...
for (let i = 0; i < mockData.length; i++) {
  ...
  const personAgeMatch = person.age >= profile.min_age_interest && person.age <= person.max_age_interest;
  const genderMatch = profile.gender_interest === person.gender;
  const personGenderMatch = person.gender_interest === profile.gender;
  const locationMatch = profile.location === person.location;

  if (ageMatch && personAgeMatch && genderMatch && personGenderMatch && locationMatch) {
    matches.push(person);
  }
}
...
if (matches.length > 0) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    console.log(`${match.first_name} ${match.last_name}, age: ${match.age}, location: ${match.location}`);
  }
  console.log(`Total matches: ${matches.length}`);
} else {
  console.log("😢 No matches found.");
}
console.log("Thank you for using our dating app! 💙");
